...
\mathematica{1.0}{ <<CoulombHiggs`}{CoulombHiggs 5.0 - A package for evaluating quiver invariants. }
...
\mathematica{1.0}{SetDirectory[NotebookDirectory[]]; <<CoulombHiggs`}{CoulombHiggs 5.0 - A package for evaluating quiver invariants.}
...
but is  released in alpha form in version 5.0, along with \cite{Pioline2019}. 
...
stack invariants $\gR(\gamma,y)$  using \cite[(4.1)]{Manschot:2013rx};}

\defn{HiggsGToOmb}{\vardef{Nvec},\newdefy }{Returns the (unevaluated) HN invariant
$\gR(\gamma,y)$ in terms of the rational refined indices $\bar\Omega(\gamma,y)$  by inverting \cite[(4.1)]{Manschot:2013rx};}
...
\defn{SubDSZ}{\vardef{Mat},\vardef{Cvec},\vardef{Li}}{gives the
                    DSZ matrix and FI parameters of the Abelian subquiver made of the charge vectors in list \var{Li};}
...
\section{Joyce-Song formula}
The Joyce-Song formula relates  rational invariants for two stability conditions:
\be
\label{JSformula}
...
\defn{JoyceSongFormula}{\vardef{Mat},\vardef{Cvec1},\vardef{Cvec2},\vardef{f}}{
...
or its analogue for stack invariants;
}
...
\defn{UFactor}{\vardef{Li},\vardef{Cvec1},\vardef{Cvec2}}{Computes the factor $U(\{\gamma_i\},\zeta_1,\zeta_2)$ defined in \cite[\S 4]{JoyceConfig}, using stability condition defined by \fun{Slope}
}

\defn{SFactor}{\vardef{Li},\vardef{Cvec1},\vardef{Cvec2}}{Computes the  factor $S(\{\gamma_i\},\zeta_1,\zeta_2)$ defined in \cite[\S 4]{JoyceConfig}, using stability condition defined by \fun{Slope}
}
...
\bibitem{Manschot:2010qz}
  ...
  [arXiv:1011.1258 [hep-th]].
  %%CITATION = doi:10.1007/JHEP07(2011)059;%%
  %87 citations counted in INSPIRE as of 20 Jun 2019
