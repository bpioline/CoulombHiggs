...
\mathematica{1.0}{ <<CoulombHiggs`}{CoulombHiggs 5.2.5 - A package for evaluating quiver invariants. }
...
\mathematica{1.0}{SetDirectory[NotebookDirectory[]]; <<CoulombHiggs`}{CoulombHiggs 5.2- A package for evaluating quiver invariants.}
...
\cite{Cordova:2013bza,Gaddam:2016xum,Messamah:2020ldn,Duan:2020qjy,Banerjee:2020moh}
...
vectors with height from $\var{Nmin}$ up to $\var{NMax}$.} 
...
\defn{NCDTSeriesFromCrystal}{\vardef{hMat}, \vardef{Framing},\vardef{Nmax}}{constructs the generating function of NCDT invariants for the quiver with height matrix 
$\var{hMat}$ and framing $\var{Framing}$ using the Quiver Yangian algorithm, for dimension
vectors with height up to $\var{NMax}$.} 
...
\defn{ChargeFunction}{\vardef{hMat},\vardef{Framing}, \vardef{Crys}, \vardef{i}, \vardef{z}}
...
\defn{VacuumChargeFunction}{\vardef{Framing}, \vardef{i}, \vardef{z}}
{provides the charge function $\Psi_{\emptyset}^i(z)=1+f_i/z$ for the full crystal, where $f_i$ is the
$i$-th entry of $\var{Framing}$. Can be redefined to accommodate non-standard vacuum 
charge functions.}


\defn{AddToCrystal}{\vardef{hMat}, \vardef{Framing}, \vardef{i}, \vardef{Crys}}{
...
\defn{GrowCrystalList}{\vardef{hMat}, \vardef{Framing}, \vardef{CrysList}}{
...
\defn{CyclicQuiverDSZ}{\vardef{avec},\vardef{t}}{constructs the DSZ matrix
